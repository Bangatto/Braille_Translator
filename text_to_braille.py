...
import filecmp
from helpers import is_capitalized
...
def is_all_caps(s):
    ...
    '''
    count = 0
    for i in s: # checking characters are valid and supported
        if not is_capitalized(i):
            return False
        else:
            count +=1
    if count < 2: # checking strings are atleast two characters
        return  False
...
def word_to_braille(word):
    ...
    if  all_caps:
        o_string += (CAPITAL+"\n\n")*2 # i multiple by 2 to concaternate capital twice
    for i in word:
        if is_digit(i):
            o_string+=NUMBER+"\n\n"
        elif (not all_caps) and i.isupper():#i checked if its not all capital but there are atleast two caps
            o_string += CAPITAL+"\n\n"

        try: #used to catch all my error Type.
            o_string += char_to_braille(i)+"\n\n"
        except TypeError:
            print("unrecognised character :"+i)
...
def new_filename(fname, addition):
    ...
    '''
    ext=fname[-4:] # it display all the last four character
    file_name = fname[:-4] + "_" + addition + ext
    return file_name
...
if __name__ == '__main__':
    doctest.testmod()
    #print(char_to_braille('«')) # used it to catch where my test was failing
